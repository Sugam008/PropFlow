...
const MapView = ({ lat, lng, address, comps }: MapViewProps) => {
  ...
  useEffect(() => {
    ...
    // Nuclear cleanup: If a map already exists on this container, remove it.
    // Leaflet stores a reference on the DOM element usually, but removing it manually is safest.
    if (mapRef.current) {
      mapRef.current.remove();
      mapRef.current = null;
    }

    // Double check if Leaflet has already attached to this container
    // @ts-ignore
    if (containerRef.current._leaflet_id) {
      // @ts-ignore
      containerRef.current._leaflet_id = null;
    }
    containerRef.current.innerHTML = ''; // Clear container completely
