...
const MapView = dynamic(() => import('../../src/components/MapView'), { 
  ssr: false,
  loading: () => <div style={{ height: '400px', width: '100%', backgroundColor: colors.gray[100], borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading Map...</div>
});
...
export default function PropertyDetailPage() {
  ...
  const followUpMutation = useMutation({
    mutationFn: ({ id, notes }: { id: string; notes: string }) => propertyApi.requestFollowUp(id, notes),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['property', id] });
      queryClient.invalidateQueries({ queryKey: ['properties'] });
  ...
  const compColumns = React.useMemo(() => [
    ...
    { header: 'Price (₹)', key: 'price', render: (item: any) => `₹${item.price.toLocaleString()}` },
    { header: 'Distance', key: 'distance_km', render: (item: any) => `${item.distance_km} km` }
  ], []);

  if (isPropertyLoading || isPhotosLoading || isCompsLoading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', gap: spacing[4] }}>
  ...
  if (!property) {
    return (
      <div style={{ padding: spacing[8], textAlign: 'center' }}>
        <div style={{ color: colors.error, marginBottom: spacing[4] }}>Property not found.</div>
        <button 
          onClick={() => router.push('/')}
          style={{ 
    ...
    <div style={{ padding: isMobile ? spacing[4] : spacing[8], maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ 
        ...
        <div>
          ...
          <h1 style={{ fontSize: isMobile ? typography.fontSizes.xl : typography.fontSizes['3xl'], fontWeight: typography.fontWeights.bold, color: colors.gray[900], marginBottom: spacing[2] }}>
            {property.address}
          </h1>
          <div style={{ 
            display: 'flex', 
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: spacing[1] }}>
              <Clock size={16} /> Submitted {new Date(property.created_at).toLocaleDateString()}
            </div>
            <Badge variant={property.status === 'SUBMITTED' ? 'info' : property.status === 'VALUED' ? 'success' : 'gray'}>
              {property.status}
            </Badge>
          </div>
        </div>
        ...
        >
          <MessageSquare size={18} aria-hidden="true" /> {isMobile ? 'Follow-up' : 'Request Follow-up'}
        </motion.button>
        <motion.button 
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
        ...
        >
          <CheckCircle2 size={18} aria-hidden="true" /> {isMobile ? 'Valuate' : 'Approve Valuation'}
        </motion.button>
      </div>
      </div>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[8] }}>
          ...
          <Card title="Property Photos">
            {photos && photos.length > 0 ? (
              <motion.div 
                ...
                style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: spacing[4] }}
              >
                {photos.map((photo: any, index: number) => (
                  <motion.div 
                    ...
                    whileHover={{ scale: 1.05, zIndex: 1, boxShadow: '0 10px 20px rgba(0,0,0,0.1)' }}
                    transition={{ type: 'spring', stiffness: 300, damping: 20 }}
                    style={{ position: 'relative', aspectRatio: '4/3', borderRadius: 8, overflow: 'hidden', border: `1px solid ${colors.border}`, cursor: 'pointer' }}
                  >
                    <Image 
                      src={photo.s3_url} 
          ...
          <Card title="Comparable Market Analysis">
            ...
            <div style={{ marginBottom: spacing[4], fontSize: typography.fontSizes.sm, color: colors.gray[500] }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[8] }}>
          ...
          <Card title="Property Details">
            <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[4] }}>
              <div>
                <div style={{ fontSize: typography.fontSizes.xs, color: colors.gray[500], textTransform: 'uppercase', marginBottom: spacing[1] }}>Property Type</div>
                <div style={{ fontWeight: typography.fontWeights.medium }}>{property.property_type}</div>
              </div>
              <div>
                <div style={{ fontSize: typography.fontSizes.xs, color: colors.gray[500], textTransform: 'uppercase', marginBottom: spacing[1] }}>Full Address</div>
                <div style={{ fontWeight: typography.fontWeights.medium }}>{property.address}</div>
                <div style={{ fontWeight: typography.fontWeights.medium }}>{property.city}, {property.state} {property.pincode}</div>
              </div>
              <div>
                <div style={{ fontSize: typography.fontSizes.xs, color: colors.gray[500], textTransform: 'uppercase', marginBottom: spacing[1] }}>Estimated Value (Customer)</div>
                <div style={{ fontWeight: typography.fontWeights.medium }}>₹{property.estimated_value?.toLocaleString() || 'Not provided'}</div>
              </div>
              <hr style={{ border: 'none', borderTop: `1px solid ${colors.border}` }} />
              <div>
                <div style={{ fontSize: typography.fontSizes.xs, color: colors.gray[500], textTransform: 'uppercase', marginBottom: spacing[1] }}>Fraud Checks</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[2], marginTop: spacing[2] }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: spacing[2], color: colors.success, fontSize: typography.fontSizes.sm }}>
                    <CheckCircle2 size={14} /> GPS Match (0.2km)
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: spacing[2], color: colors.success, fontSize: typography.fontSizes.sm }}>
                    <CheckCircle2 size={14} /> EXIF Data Verified
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: spacing[2], color: colors.success, fontSize: typography.fontSizes.sm }}>
                    <CheckCircle2 size={14} /> Live Capture Verified
                  </div>
                </div>
              </div>
            </div>
      ...
      >
        <form onSubmit={handleValuationSubmit} style={{ display: 'flex', flexDirection: 'column', gap: spacing[4] }}>
          <div>
            <label style={{ display: 'block', fontSize: typography.fontSizes.sm, fontWeight: typography.fontWeights.medium, color: colors.gray[700], marginBottom: spacing[1] }}>
            ...
            <input 
              ...
              onChange={(e) => setValuationData({ ...valuationData, estimated_value: parseInt(e.target.value) })}
              style={{ 
                width: '100%', 
                padding: spacing[3], 
                borderRadius: 8, 
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing[4] }}>
            <div>
              ...
              <input 
                ...
                onChange={(e) => setValuationData({ ...valuationData, confidence_score: parseInt(e.target.value) })}
                style={{ 
                  width: '100%', 
                  padding: spacing[3], 
                  borderRadius: 8, 
            ...
            <div>
              ...
              <select 
                ...
                onChange={(e) => setValuationData({ ...valuationData, methodology: e.target.value })}
                style={{ 
                  width: '100%', 
                  padding: spacing[3], 
                  borderRadius: 8, 
          ...
          <div style={{ 
            ...
            <strong>Note:</strong> Submitting this valuation will notify the customer and update the property status to &quot;VALUED&quot;.
          </div>
        </form>
      </Modal>
          ...
          <>
            ...
            <button 
              ...
              style={{ 
                ...
                padding: `${spacing[2]}px ${spacing[4]}px`,
                borderRadius: 8,
                cursor: (followUpMutation.isPending || !followUpNotes.trim()) ? 'not-allowed' : 'pointer',
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[4] }}>
          <p style={{ fontSize: typography.fontSizes.sm, color: colors.gray[600] }}>
          ...
          <div style={{ 
            ...
            <div>
              <strong>Note:</strong> This will set the property status to &quot;FOLLOW_UP&quot; and notify the customer via SMS/WhatsApp.
            </div>
          </div>
        </div>
      </Modal>
      ...
      >
        <div style={{ textAlign: 'center', padding: `${spacing[4]}px 0` }}>
          ...
          <h2 style={{ fontSize: typography.fontSizes.xl, fontWeight: typography.fontWeights.bold, color: colors.gray[900], marginBottom: spacing[2] }}>
            Submission Successful
          </h2>
          <p style={{ color: colors.gray[600], fontSize: typography.fontSizes.base, lineHeight: 1.5 }}>
            The property record has been updated and the customer has been notified. What would you like to do next?
          </p>
        </div>
      </Modal>
    </div>
